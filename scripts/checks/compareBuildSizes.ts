...
async function compareBuildSizes() {
  const nextSizes: StatMap = JSON.parse(
    fs.readFileSync('./packages/sizes.json', 'utf8')
  )
  ...
  try {
    const request = await fetch(
      'https://raw.githubusercontent.com/lorica/uc-design-system/master/packages/sizes.json'
    )
