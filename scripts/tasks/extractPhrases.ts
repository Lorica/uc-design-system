...
const I18N_PREFIX = (process.env.I18N_PREFIX || 'uc-design-system').split(',')
  ...
  .then((files) => {
    ...
    const handleExtract: OnExtract = (key, message, context) => {
      if (!I18N_PREFIX.some((prefix) => key.startsWith(prefix))) {
        throw new Error(
          `Key "${key}" does not start with "${I18N_PREFIX.join(', ')}".`
        )
      }
      ...
      if (phrase) {
        ...
        if (dupe && !dupe.includes(key)) {
          console.warn(
            chalk.yellow(
              `Duplicate phrase message found for keys: ${dupe.join(', ')}`
            )
          )
