export default function getImportPath(filePath, baseName) {
  const indexParts = filePath.match(
    /^packages\/([\w-]+)\/src\/index\.tsx?#(\w+)$/i
  )
  ...
  const parts = filePath.match(
    /^packages\/([\w-]+)\/src\/components\/(\w+)(\/\w+)?\.tsx#(\w+)$/i
  )
  ...
  const packageName =
    pkgName === 'core' ? 'uc-design-system' : `uc-design-system-${pkgName}`
