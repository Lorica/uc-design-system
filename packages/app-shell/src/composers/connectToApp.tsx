...
export default function connectToApp(scope: string) /* infer */ {
  ...
  return function connectToAppFactory<Props extends object = {}>(
    ...
    class ConnectToApp extends React.Component<
      ...
      getPageData(): object {
        ...
        if (current && typeof current.registerPageData === 'function') {
          ...
          throw new Error(
            'Connected component has not defined an registerPageData() method.'
          )
        }
    ...
    function ConnectToAppWrapper(props: Props) {
      return (
        <AppContext.Consumer>
          {(app) => <ConnectToApp {...props} app={app} />}
        </AppContext.Consumer>
      )
    }

    return withBoundary(scope)(
      finishHOC('connectToApp', ConnectToAppWrapper, WrappedComponent)
    )
  }
}
