...
describe('<Composer />', () => {
  ...
  it('renders a `beforeButton`', () => {
    ...
    const { root } = render<ComposerProps>(
      <Composer {...props} beforeButton={node} />
    )
  ...
  it('renders an `afterButton`', () => {
    const node = <EmojiButton />
    const { root } = render<ComposerProps>(
      <Composer {...props} afterButton={node} />
    )
  ...
  describe('states', () => {
    ...
    it('marks invalid if `onSubmit` throws an error', () => {
      ...
      root
        .findOne('textarea')
        .dispatch('onChange', { target: { value: 'foo' } })
  ...
  describe('hotkeys', () => {
    it('executes a custom hotkey', () => {
      ...
      root
        .findOne('textarea')
        .dispatch('onKeyDown', { key: 'Enter', shiftKey: true })
  ...
  describe('actions', () => {
    function openActionMenu(
      ...
      root
        .findOne('button', { id: 'toggle-button-actions' })
        .dispatch('onClick')
    }
  ...
  describe('emojis', () => {
    async function openEmojiMenu(
      ...
      await root
        .findOne('button', { id: 'toggle-button-emojis' })
  ...
  describe('previews', () => {
    async function openPreviewMenu(
      ...
      await root
        .findOne('textarea')
        .dispatchAndWait('onChange', { target: { value } })

      await root
        .findOne('button', { id: 'composer-submit-button' })
        .dispatchAndWait('onClick')
    }
    ...
    it('opens preview menu when form is submitted and does not submit entirely', async () => {
      ...
      expect(
        root.findOne(Menu, { name: MENU_PREVIEW }).findOne(Interweave)
      ).toHaveProp('content', 'Hello')
