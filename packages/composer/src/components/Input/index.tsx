...
export default function Input({
  ...
  let placeholder =
    messagePlaceholder ?? T.phrase('uc-design-system.composer.labels.sendMessage', 'Send message…');

  if (context.mode === MODE_EMAIL) {
    placeholder =
      emailPlaceholder ?? T.phrase('uc-design-system.composer.labels.sendEmail', 'Send email…');
  } else if (context.mode === MODE_PRIVATE_NOTE) {
    placeholder =
      privateNotePlaceholder ??
      T.phrase('uc-design-system.composer.labels.privateToAirbnb', 'Private to Airbnb');
  }
  ...
  return (
    <>
      {context.mode === MODE_EMAIL && (
        <>
          <InlineInput
            label={T.phrase('uc-design-system.composer.email.subjectLine', 'Re:')}
            name="emailSubject"
      ...
      >
        ...
        <Hotkey
          ...
          label={
            context.flags.previewConfirm
              ? T.phrase('uc-design-system.composer.hotkey.returnToPreview', 'to preview')
              : T.phrase('uc-design-system.composer.hotkey.returnToSend', 'to send')
          }
          order={100}
          onRun={handleSubmit}
        />
        ...
        <Hotkey
          ...
          name="closeMenu"
          label={T.phrase('uc-design-system.composer.hotkey.toDismiss', 'to dismiss')}
          onRun={closeMenu}
        />
      </div>
    </>
