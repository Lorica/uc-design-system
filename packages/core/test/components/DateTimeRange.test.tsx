...
import { wrapEnv } from '@lorica/uc-design-systemsign-systemsign-systemsign-system-test-utils'
...
describe('<DateTimeRange />', () => {
  ...
  it('errors for an invalid time', () => {
    ...
    expect(() =>
      shallow(<DateTimeRange from={dateA} to="2016-02-33" />)
    ).toThrow('Invalid Date')
  })
  ...
  it('formats correctly for different days', () => {
    const wrapper = mount(
      <DateTimeRange from={dateA} to={dateB} timezone="UTC" />
    )
  ...
  it('formats correctly for different months', () => {
    const wrapper = mount(
      <DateTimeRange from={dateA} to={dateC} timezone="UTC" />
    )
  ...
  it('formats correctly for different years', () => {
    const wrapper = mount(
      <DateTimeRange from={dateA} to={dateD} timezone="UTC" />
    )

    expect(wrapper.text().replace(/\s+/g, ' ')).toBe(
      'Feb 26, 1988 – Jul 23, 1989'
    )
  })

  it('formats correctly for the same day', () => {
    const wrapper = mount(
      <DateTimeRange from={dateC} to={dateC} timezone="UTC" />
    )
  ...
  it('formats correctly using no timezone', () => {
    const wrapper = mount(
      <DateTimeRange from={dateA} to={dateD} timezone={false} />
    )
