...
describe('<TextArea />', () => {
  it('renders a field and input', () => {
    const wrapper = shallow(
      <TextArea name="foo" label="Label" onChange={() => {}} />
    )
  ...
  it('generates a unique ID', () => {
    ...
    expect(wrapper.find(FormField).prop('id')).toBe(
      wrapper.find(BaseTextArea).prop('id')
    )
  })

  it('can set `noTranslate`', () => {
    const wrapper = shallow(
      <TextArea noTranslate name="foo" label="Label" onChange={() => {}} />
    )
  ...
  it('can set rows', () => {
    const wrapper = shallow(
      <TextArea name="foo" label="Label" rows={5} onChange={() => {}} />
    )
  ...
  it('sets a label description when max length is used', () => {
    ...
    expect(
      (wrapper.find(FormField).prop('labelDescription') as React.ReactElement)
        .props
    ).toEqual({
      k: 'uc-design-system.form.charsUsed',
