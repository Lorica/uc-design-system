...
describe('<FileInput />', () => {
  ...
  it('generates a unique ID', () => {
    ...
    expect(wrapper.find(FormField).prop('id')).toBe(
      wrapper.find(FormInput).prop('id')
    )
  })
  ...
  it('defaults to any files', () => {
    ...
    expect(
      (wrapper.find(FormInputButton).prop('afterIcon') as React.ReactElement)
        .type
    ).toBe(IconUpload)
  ...
  it('shows file count in button when files are selected', () => {
    ...
    expect(wrapper.find(FormInputButton).find('span').prop('children')).toBe(
      ' (1)'
    )
  })
  ...
  it('can hide all columns except name and remove', () => {
    const wrapper = shallow(
      <FileInput {...props} hideLastModified hideFileSize hideFileType />
    )
  ...
  it('clicking the button triggers the input', () => {
    ...
    // @ts-ignore
    wrapper.instance().ref = { current: { click: spy } }
