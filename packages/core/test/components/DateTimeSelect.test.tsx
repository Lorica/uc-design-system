...
describe('<DateTimeSelect />', () => {
  ...
  describe('getCurrentValue()', () => {
    ...
    beforeEach(() => {
      wrapper = shallowWithStyles<BaseDateTimeSelect>(
        <DateTimeSelect {...props} value={date} />
      )
    ...
    it('returns year', () => {
      expect(instance.getCurrentValue(wrapper.state('date'), 'year')).toBe(
        '1988'
      )
    })
    ...
    it('returns minute', () => {
      expect(instance.getCurrentValue(wrapper.state('date'), 'minute')).toBe(
        '0'
      )
    })

    it('returns meridiem (from state)', () => {
      expect(instance.getCurrentValue(wrapper.state('date'), 'meridiem')).toBe(
        'pm'
      )
  ...
  describe('handleChange()', () => {
    function simulateChange<P>(
      wrapper: Enzyme.ShallowWrapper<P>,
      type: string,
      value: unknown
    ) {
      const event = {
      ...
      wrapper
        .findWhere(
          (node) =>
            node.type() === BaseSelect && node.prop('name') === `foo[${type}]`
        )
    ...
    it('calls `onChange` prop', () => {
      const spy = jest.fn()
      const wrapper = shallowWithStyles(
        <DateTimeSelect {...props} value={date} onChange={spy} />
      )
