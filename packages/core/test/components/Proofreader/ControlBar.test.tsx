...
import ControlBar, {
  ControlBarProps,
} from '../../../src/components/Proofreader/ControlBar'
...
describe('<ControlBar />', () => {
  ...
  it('selects locale when clicking an item', () => {
    const spy = jest.fn()
    const { root } = render<ControlBarProps>(
      <ControlBar {...props} onSelectLocale={spy} />
    )
  ...
  it('shows error count when list is non-empty', () => {
    const { root, update } = render<ControlBarProps>(<ControlBar {...props} />)
    const k = 'uc-design-system.proofreader.totalIssues'
  ...
  it('handles unknown locale', () => {
    const { root } = render<ControlBarProps>(
      <ControlBar {...props} locale="foo" />
    )

    expect(
      root.find(T, { k: 'uc-design-system.proofreader.unsupportedLanguage' })
    ).toHaveLength(1)
  })

  it('handles no locale', () => {
    const { root } = render<ControlBarProps>(
      <ControlBar {...props} locale="none" />
    )
