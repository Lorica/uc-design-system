...
export function getLocaleDefinition(locale: string): LocaleDefinition {
  if (locale === NO_LOCALE) {
    return {
      locale,
      label: T.phrase('uc-design-system.proofreader.noLanguageSelected', 'No language selected'),
    };
  }
  ...
  if (locale === AUTO_DETECT_LOCALE) {
    return {
      locale,
      label: T.phrase('uc-design-system.proofreader.autoDetectLanguage', 'Auto-detect language'),
    };
  }
  ...
  while (match) {
    if (match[0] !== 'Airbnb') {
      customErrors.push({
        short_message: '',
        message: T.phrase(
          'uc-design-system.proofreader.misspellingLabel',
