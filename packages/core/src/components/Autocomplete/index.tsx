...
import renderElementOrFunction, {
  RenderableProp,
} from '../../utils/renderElementOrFunction'
...
export type AutocompleteProps<T extends Item = Item> = Omit<
  BaseInputProps,
  'id'
> &
  FormFieldProps & {
...
export default class Autocomplete<
  T extends Item = Item
> extends React.Component<AutocompleteProps<T>, AutocompleteState<T>> {
  static defaultProps = {
    autoFocus: false,
    clearOnSelect: false,
    debounce: 250,
  ...
  componentDidUpdate(
    prevProps: AutocompleteProps<T>,
    prevState: AutocompleteState<T>
  ) {
  ...
  getInputProps(
    props: AutocompleteProps<T> & { hasPrefix: boolean; hasSuffix: boolean }
  ) {
    const {
      disabled,
      invalid,
    ...
    return {
      ...
      optional,
      placeholder:
        placeholder ?? T.phrase('uc-design-system.common.search', 'Search'),
      small,
      large,
      hasPrefix,
      hasSuffix,
  ...
  maybeAutoCompleteText = (state: AutocompleteState<T>) => {
    ...
    const matchedItem =
      items[index] && isItemSelectable!(items[index]) ? items[index] : name
        ...
        {renderElementOrFunction(this.props.renderNoResults) || (
          <Text>
            {this.props.noResultsText || (
              <T
                k="uc-design-system.common.noResults"
