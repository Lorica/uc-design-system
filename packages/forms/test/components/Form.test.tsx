...
describe('<Form />', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(
      <Form onSubmit={(data) => Promise.resolve(data)}>Hello</Form>
    )
  ...
  it('renders function children', () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ initialValues: { foo: 'bar' } })
    )
  })
  ...
  describe('prepareData()', () => {
    ...
    it('parses strings', () => {
      expect(instance.prepareData({ foo: 123 })).toEqual(
        expect.objectContaining({ foo: '123' })
      )
    })

    it('parses numbers', () => {
      expect(instance.prepareData({ bar: '123' })).toEqual(
        expect.objectContaining({ bar: 123 })
      )
    })

    it('parses booleans', () => {
      expect(instance.prepareData({ baz: 0 })).toEqual(
        expect.objectContaining({ baz: false })
      )
    })

    it('parses all', () => {
      expect(
        instance.prepareData({ foo: ' foo ', bar: '123.45', baz: 1 })
      ).toEqual({
        foo: 'foo',
        bar: 123.45,
        baz: true,
